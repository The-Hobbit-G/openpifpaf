        ...
        for batch_i, item in enumerated_dataloader:
            ...
            for image, pred, gt_anns, meta in \
                ...
                for pr_ann,pred_point in zip(pred,pred_points):
                    ...
                    # cv2.rectangle(image, (int(top_left_point[0]), int(pred_box[1])), (int(bottom_right_point[0]), int(bottom_right_point[1])), (0, 255, 255), 2)
                    cv2.putText(image, str(pred_category), (int(top_left_point[1]), int(pred_box[1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
